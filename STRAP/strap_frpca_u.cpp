...
int main(int argc,  char **argv){
  ...
  string outUfile = EBpath + queryname + "_strap_frpca_u_U.csv";
  string outVfile = EBpath + queryname + "_strap_frpca_u_V.csv";
  ...
  auto elapsed_deque_to_sparse_time = chrono::duration_cast<std::chrono::seconds>(hash_coo_time - merge_ppr_time);
  ...
  for(int i = 0; i < g.n; i++){
    for(int j = 1; j < d; j++){
      double val = matrix_get_element(V, i, d-j-1)*sqrt(abs(S->d[d-j-1]));
      outV << val << ", ";
    }
    double val_last = matrix_get_element(V, i, d-1)*sqrt(abs(S->d[d-1]));
    outV << val_last << endl;
  }
